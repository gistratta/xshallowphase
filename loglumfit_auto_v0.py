...
"""
outfileold_E0=path+"E0_Long_GoldenGRB.txt"
if os.path.isfile(outfileold_E0):
	os.system('rm '+outfileold_E0)
if not os.path.isfile(outfileold_E0):
	os.system('touch '+outfileold_E0)
	out_file1 = open(outfileold_E0,"a")
	out_file1.write("GRB,Tstart,E051,chisq,dof,P-value"+"\n")
	out_file1.close()
"""
...
for fi in listgrb:
    ...
    # --- legge l'energia E0
    fileinE0=path+'E0_Long_GoldenGRB.txt'
    dataE0=pd.read_csv(fileinE0,comment='!', sep=',', header=None,skiprows=None,skip_blank_lines=True)
    dataE0_array=dataE0.values
    print "Index for ",fi," : ", np.where(dataE0_array==fi)
    datafi0=dataE0_array[np.where(dataE0_array[:,0]==fi)]
    datafi0flat=datafi0.flatten()
    E0=float(datafi0flat[2])
    print "E0=",E0
    ...
    #    print "E0 [",E0,"(10^51 erg)] =", "%.5f" %popt[3], "+/-", "%.5f" %pcov[3,3]**0.5
        ...
        print " E0=(Li-L(Tt))*Tstart/k=",(Li-10**(model(np.log10(startTxrt),popt[0],popt[1],popt[2])))*startTxrt/popt[0]
        print "------  "
    ...
    #    bfmodel=model(np.log10(t),popt[0],popt[1],popt[2],popt[3])
        ...
        out_file.write(fi+","+str(startTxrt)+","+str(E0)+","+str("%.5f" %popt[0])+","+str("%.5f" %pcov[0,0]**0.5)+","+str("%.5f" %popt[1])+","+str("%.5f" %pcov[1,1]**0.5)+","+str("%.5f" %Pms)+","+str("%.5f" %dPms)+","+str("%.5f" %mychi)+","+str("%.5f" %dof)+","+str("%.5f" %p_value)+"\n")
        out_file.close()

        """
        out_file1 = open(outfileold_E0,"a")
        out_file1.write(fi+","+str(startTxrt)+","+str(E051)+","+str("%.5f" %mychi)+","+str("%.5f" %dof)+","+str("%.5f" %p_value)+"\n")
        out_file1.close()
        """
...
#        out_file.write(fi+","+str(startTxrt)+","+str(E051)+","+str("%.5f" %popt[0])+","+str("%.5f" %pcov[0,0]**0.5)+","+str("%.5f" %popt[1])+","+str("%.5f" %pcov[1,1]**0.5)+","+str("%.5f" %popt[2])+","+str("%.5f" %pcov[2,2]**0.5)+","+str("%.5f" %popt[3])+","+str("%.5f" %pcov[3,3]**0.5)+","+str("%.5f" %mychi)+","+str("%.5f" %dof)+","+str("%.5f" %p_value)+"\n")
        out_file.write(fi+","+str(startTxrt)+","+str(E0)+","+str("%.5f" %popt[0])+","+str("%.5f" %pcov[0,0]**0.5)+","+str("%.5f" %popt[1])+","+str("%.5f" %pcov[1,1]**0.5)+","+str("%.5f" %Pms)+","+str("%.5f" %dPms)+","+str("%.5f" %popt[3])+","+str("%.5f" %pcov[3,3]**0.5)+","+str("%.5f" %mychi)+","+str("%.5f" %dof)+","+str("%.5f" %p_value)+"\n")
        out_file.close()
    ...
    #    popt, pcov = curve_fit(model, x, y, p0, sigma=dy, bounds=([0.001,0.0001,0.001,0.00001,0.0], [4.0, 100.,50.0,100.0,1.0]))
        ...
        out_file = open(outfilenewx,"a")
        out_file.write(fi+","+str(startTxrt)+","+str(E0)+","+str(alphax)+","+str("%.5f" %popt[0])+","+str("%.5f" %pcov[0,0]**0.5)+","+str("%.5f" %popt[1])+","+str("%.5f" %pcov[1,1]**0.5)+","+str("%.5f" %Pms)+","+str("%.5f" %dPms)+","+str("%.5f" %mychi)+","+str("%.5f" %dof)+","+str("%.5f" %p_value)+"\n")
        out_file.close()
